    ...
    <div className="fixed inset-0 z-50 overflow-hidden select-none">
      ...
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md transform transition-all duration-300 ease-out">
          <div className="h-full flex flex-col bg-cream-100 border-l border-cream-400 shadow-2xl overflow-hidden">
            {/* Header */}
            <div className="px-6 py-5 bg-gradient-to-r from-cream-300 to-cream-200 border-b border-cream-400/40 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="p-2 rounded-xl bg-saffron-500 text-white shadow-xs">
                  <Bookmark className="w-4 h-4" />
                </div>
                <div>
                  <h2 className="font-cinzel font-bold text-lg text-saffron-800">
                    Saved Sanctuary
                  </h2>
                  <p className="text-[10px] text-stone-500 font-sans tracking-wide uppercase">
              ...
              <button
                onClick={onClose}
                className="p-1.5 rounded-full text-stone-500 hover:text-saffron-700 hover:bg-cream-400/40 transition-colors cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 px-6 py-6 overflow-y-auto space-y-4">
              {bookmarks.length === 0 ? (
                <div className="text-center py-16 space-y-3">
                  <div className="w-12 h-12 rounded-full bg-cream-300 border border-cream-400 flex items-center justify-center mx-auto text-stone-400">
                    <BookOpen className="w-6 h-6 text-saffron-600/60" />
                  </div>
                  <p className="font-cinzel font-bold text-sm text-stone-700">
                    No Saved Verses Yet
                  </p>
                ...
                bookmarks.map((item) => (
                  <div
                    ...
                    className="group relative p-4 bg-white rounded-2xl border border-cream-400 hover:border-saffron-500/40 shadow-xs hover:shadow-md transition-all duration-200 cursor-pointer space-y-2"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold text-saffron-600 bg-saffron-50 px-2.5 py-0.5 rounded-full border border-saffron-200/40 uppercase tracking-wider">
                        {item.source_name}
                      </span>
                    ...
                    <h4 className="font-cinzel font-bold text-xs text-stone-800">
                      Ch. {item.chapter_number}, Verse {item.verse_number}
                    </h4>

                    <p className="font-cinzel text-xs text-stone-700 font-semibold line-clamp-2">
                      {item.sanskrit_text}
                    </p>

                    {item.translation_text && (
                      <p className="font-serif italic text-xs text-stone-500 line-clamp-2">
                        &quot;{item.translation_text}&quot;
                      </p>
                    )}
                  </div>
            ...
            {/* Footer */}
            <div className="px-6 py-4 bg-cream-200/60 border-t border-cream-400/40 text-center text-[10px] text-stone-500">
