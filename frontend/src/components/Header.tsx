...
import { 
  Info, X, ExternalLink, Globe, Bookmark, Compass, BookOpen, Sun, Moon 
} from 'lucide-react';
...
export default function Header({ onOpenSanctuary, mode, onModeChange, onHomeClick }: HeaderProps) {
  ...
  return (
    <>
      ...
      {/* Slide-over Drawer for About (Portaled to document.body) */}
      {isAboutOpen && mounted && createPortal(
        ...
        >
          {/* Backdrop Blur */}
          <div 
            className="fixed inset-0 bg-stone-900/50 dark:bg-black/80 backdrop-blur-sm transition-opacity duration-300" 
            onClick={() => setIsAboutOpen(false)}
          />

          <div className="relative w-full max-w-md h-full bg-cream-100 dark:bg-[#0b0f19] border-l border-cream-400 dark:border-amber-500/20 shadow-2xl flex flex-col z-10 animate-fade-in">
            {/* Header */}
            <div className="px-6 py-6 bg-gradient-to-r from-cream-300 to-cream-200 dark:from-[#111827] dark:to-[#0b0f19] border-b border-cream-400/40 dark:border-amber-500/20 flex items-center justify-between shrink-0">
              <h2 id="about-modal-title" className="text-2xl font-bold font-cinzel text-saffron-700 dark:text-amber-300">
                About DharmaPragya
              </h2>
              <button
                onClick={() => setIsAboutOpen(false)}
            ...
            {/* Content */}
            <div className="flex-1 px-6 py-8 space-y-8 overflow-y-auto select-text">
            ...
            <div className="px-6 py-6 bg-cream-200/50 dark:bg-slate-900/90 border-t border-cream-400/30 dark:border-amber-500/20 text-center text-[10px] text-stone-500 dark:text-slate-400 font-medium shrink-0">
